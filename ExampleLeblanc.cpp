...
int main(int, char**)
{
	...
	FVM_1D numericSolver(U, -10.0, 10.0);
	numericSolver.setEndingTime(0.0001);
	...
	if (oFileRho_ && oFileP_ && oFileU_)
	{
		...
		{
			oFileRho_ << solver(0.02 * double(i - 500), 0.0001)(0) << ' ';
			oFileP_ << solver(0.02 * double(i - 500), 0.0001)(1) << ' ';
			oFileU_ << solver(0.02 * double(i - 500), 0.0001)(2) << ' ';
		}
		oFileRho_.close();
		oFileP_.close();
		oFileU_.close();
